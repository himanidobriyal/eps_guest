...
/* 🔥 FINAL FIX: dropdown options text invisible issue 
#puSelect option,
#deptSelect option,
#orgSelect option,
#unitDropdown option {
...
/* 🔥 override accessibility forced colors 
select {
  forced-color-adjust: none;
}*/
/* 🔥 HARD OVERRIDE for dropdown option text */
/* 🚑 EMERGENCY FIX: invisible dropdown options */
...
/* accessibility / high contrast kill switch */
select,
select option {
...
/* 🔥 FIX: dropdown options text invisible issue */
select option {
  color: #000 !important;
  background: #fff !important;
}

/* Chrome + accessibility fix */
select option:checked,
select option:hover {
...
<div id="mySidebar" class="sidebar">
 ...
 action="${pageContext.request.contextPath}/eps/searchtender/customSearch">
	...
	<input type="radio" name="searchType" id="tenderNo"  />
...
<div class="search-criteria-section" id="searchMethodSection" style="display:none;">
  <div class="section-label">Search Method</div>
  <div class="criteria-options">

    <div class="criteria-option" id="method-startsWith">
      <input type="radio" name="searchMethod" id="startsWith" />
    ...
    <div class="criteria-option" id="method-endsWith">
...
<footer class="footer">
  ...
  $(document).ready(function () {
    ...
    $("#puSelect").html('<option value="">-- Select Zone --</option>');
	...
	// 🔥 Search Criteria → placeholder + method control
document.querySelectorAll('input[name="searchType"]').forEach(radio => {
  radio.addEventListener('change', function () {

    // 1️⃣ Update placeholder
    ...
    else if (this.id === 'itemCode') searchInput.placeholder = 'Enter item code...';

    // 2️⃣ 🔑 Show correct Search Method options
    handleSearchCriteriaChange(this.id);
  });
});
	...
	function handleSearchCriteriaChange(criteriaId) {
		  ...
		  const methodSection = document.getElementById("searchMethodSection");

		  const startsWith = document.getElementById("method-startsWith");
		  const containing = document.getElementById("method-containing");
		  const endsWith = document.getElementById("method-endsWith");

		  // safety
		  if (!methodSection || !startsWith || !containing || !endsWith) return;

		  // show Search Method section
		  methodSection.style.display = "block";

		  // 🔥 hide all methods first
		  startsWith.style.display = "none";
		  containing.style.display = "none";
		  endsWith.style.display = "none";
		  ...
		  // Tender Number → Starting with ONLY
		  if (criteriaId === "tenderNo") {
		    startsWith.style.display = "flex";
		  }

		  // Tender Title → Starting with + Containing
		  else if (criteriaId === "tenderTitle") {
		    startsWith.style.display = "flex";
		    containing.style.display = "flex";
		  }

		  // Item Description → Starting with + Containing
		  else if (criteriaId === "itemDesc") {
		    startsWith.style.display = "flex";
		    containing.style.display = "flex";
		  }
		}
	...
	$("#orgSelect").on("change", function () {
		  ...
		  console.log("ORG SELECTED =", orgCode);
		  ...
		  // PU
		  $.getJSON(baseUrl + "/eps/searchtender/getPUs", { orgCode })
		    .done(function (data) {
		      console.log("PU DATA =", data);

		      let pu = $("#puSelect");
		      pu.append('<option value="ALL">All</option>');

		      data.forEach(p => {
		        pu.append(`<option value="${p.intCode}">${p.value}</option>`);
		      });
		    })
		    .fail(function (err) {
		      console.error("PU API FAILED", err);
		    });
		    
		  

		  // Department
		  /*$.getJSON(baseUrl + "/eps/searchtender/getDepartments", { orgCode })
		    .done(function (data) {
		      console.log("DEPT DATA =", data);

		      let d = $("#deptSelect");
		      d.append('<option value="ALL">All</option>');

		      data.forEach(dep => {
		        d.append(`<option value="${dep.intCode}">${dep.value}</option>`);
		      });
		    })
		    .fail(function (err) {
		      console.error("DEPT API FAILED", err);
		    });

		});*/
		$.getJSON(baseUrl + "/eps/searchtender/getDepartments", { orgCode }, function (data) {
			  ...
			  data.forEach(dep => {
			    d.append(
			      '<option value="' + dep.intCode + '">' + dep.value + '</option>'
			    );
			  });
  ...
  // 1️⃣ Placeholder (default)
  puDropdown.append('<option value="">-- Select Zone --</option>');

  // 2️⃣ All option
  puDropdown.append('<option value="ALL">All</option>');
  ...
  // All
  deptDropdown.append('<option value="ALL">All</option>');
...
$(document).ready(function () {
  ...
  // agar organization already selected hai
  if ($("#orgSelect").val()) {
    console.log("FORCING ORG CHANGE");
