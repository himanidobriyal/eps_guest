...
<div class="main-container">
	<div class="search-card">
		...
		<div class="search-header">Query Search</div>
				...
				<fieldset>
					...
					<div class="form-row">
						...
						<div>
							...
							<div id="queryError" class="error-text"></div>
						</div>
    ...
    <button type="reset" class="btn btn-secondary">Cancel</button>
				...
				<div class="no-record" id="resultMsg">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function clearErrors() {
        ...
        errorBox.innerHTML = "";
        queryId.classList.remove("input-error");
    ...
    // =========================
    queryId.addEventListener("input", function () {

        // Trim & remove all spaces inside
        queryId.value = queryId.value.replace(/\s+/g, "");

        clearErrors();
        resultMsg.style.display = "none";

        const value = queryId.value.trim();

        if (value.length > 0) {

            if (value.length < 3) {
                showError("Query ID must be minimum 3 characters.");
                queryId.classList.add("input-error");
            }
            else if (value.length > 50) {
                showError("Query ID cannot exceed 50 characters.");
                queryId.classList.add("input-error");
            }
            else if (!queryRegex.test(value)) {
                showError("Only alphanumeric characters and hyphen (-) allowed.");
                queryId.classList.add("input-error");
            }
        }
    });
    ...
    form.addEventListener("submit", function (e) {
        ...
        let queryValue = queryId.value.trim().replace(/\s+/g, "");
        let emailValue = emailId.value.trim();

        queryId.value = queryValue;

        if (queryValue === "" && emailValue === "") {
            errors.push("Please enter either Query ID or Email ID.");
            queryId.classList.add("input-error");
            emailId.classList.add("input-error");
        }
        ...
        if (queryValue !== "") {
            if (queryValue.length < 3) {
                errors.push("Query ID must be minimum 3 characters.");
                queryId.classList.add("input-error");
            }
            else if (queryValue.length > 50) {
                errors.push("Query ID cannot exceed 50 characters.");
                queryId.classList.add("input-error");
            }
            else if (!queryRegex.test(queryValue)) {
                errors.push("Only alphanumeric characters and hyphen (-) allowed in Query ID.");
                queryId.classList.add("input-error");
            }
        }
    ...
    function showError(message) {
        errorBox.innerHTML = "<ul><li>" + message + "</li></ul>";
        errorBox.style.display = "block";
    }
